...
\subsection{VIL implementation of Quote determination}

\noindent Load asset weights, names, and makret asset names:        
...
\noindent Store results $P$, $S$, and $C$:
...
\noindent \small The above code snippet is included for demonstration purposes. For most up to date version of the code please refer to project repository.
...
\pagebreak
\subsection{VIL implementation of Supply and Delta update}

\noindent Load asset names:
...
\pagebreak
\subsection{VIL implementation of Market Data update}

\noindent Update Prices, Slopes, and Liquidity:
...
\noindent Store asset Prices, Slopes, and Liquidity:
...
\noindent The order's spent collateral is calculated: 

\[
    C_{\text{spent}} = Q_{\text{final}} \cdot (S \cdot Q_{\text{final}} + P)
\]

\noindent All market and order state vectors are then committed.

\subsection{VIL implementation of Index Order execution}

\noindent Load asset weights:
...
\noindent Cap $MaxOrderSize$:
\[
    C_{\text{eff}} = \min(\text{MaxOrderSize}, C_{\text{new}})
\]

\noindent and Solve Quadratic:
\[
    S \cdot Q^2 + P \cdot Q - C_{\text{eff}} = 0
\]
\begin{verbatim}
...
\noindent Cap Index Quantity with Capacity:
\[
    Q_{\text{final}} = \min(Q_{\text{max, pricing}}, CL)
\]
...
\noindent Generate individual Asset Orders (compute asset quantities):
\[
    \mathbf{A} = Q_{\text{final}} \odot \mathbf{W}
\]
...
\noindent Load asset names \textsl{(for Index order and for market)}:
...
\noindent Compute Index Order total remaining Collateral:
\[
...
\noindent Compute Index order total spent Collateral: 
\[
...
\noindent Store updated Index order:
...
\noindent Store executed Index quantity and remaining quantity:
...
\subsection*{Rust implementation of adaptive Join-Filter operation}
