...
show_help() {
    ...
    echo "  --no-granary                  Skip Granary deployment and attachment"
    echo "  --only-granary <CASTLE_ADDR>  Skip everything except Granary deployment and link to CASTLE_ADDR"
    echo ""
    ...
    echo "  ./castle.sh --no-castle 0x123...        # Add officers/granary to existing castle"
    echo "  ./castle.sh --only-granary 0x123...     # Only deploy and attach a new Granary"
    exit 0
}
...
if [ "$NO_CASTLE" = true ] || [ "$ONLY_GRANARY" = true ]; then
    if [ -z "$CASTLE_TARGET_ADDR" ]; then
        die "Error: --no-castle or --only-granary requires a <CASTLE_ADDRESS>"
    fi
...
if [ "$DO_CONSTABLE" = false ] && [ "$ONLY_GRANARY" = false ]; then
...
if [ "$SKIP_GRANARY" = false ]; then
    ...
    contract_send "$TARGET_ADDRESS" "appendGranary(address)" "$GRANARY_TARGET"
fi
...
echo "---------------------------"
echo "Castle Target: $TARGET_ADDRESS"
[ "$SKIP_GRANARY" = false ] && echo "Granary Target: $GRANARY_TARGET"
echo "---------------------------"